...
export function GenderChart({ data }: GenderChartProps) {
    ...
    if (total === 0) return (
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm flex flex-col h-full">
            <SectionHeader title="Gender Distribution" icon={PieChart} />
            <div className="flex-1 flex items-center justify-center p-8 text-muted-foreground italic text-xs font-medium">No data available</div>
        </div>
    );
    ...
    return (
        <div className="bg-card rounded-xl border border-border shadow-sm flex flex-col items-center transition-all hover:shadow-md h-full">
            <SectionHeader title="Gender Distribution" icon={PieChart} className="w-full" />
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 w-full">
                <div className="relative w-45 h-45">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        ...
                        <span className="text-[10px] uppercase font-black text-muted-foreground tracking-widest opacity-60">Total</span>
                    </div>
                </div>
                ...
                <div className="mt-8 grid grid-cols-2 gap-x-8 gap-y-3 w-full max-w-50">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-foreground uppercase tracking-tight">Male: {malePerc.toFixed(1)}%</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-(--chart-pink)" />
                        <span className="text-[10px] font-black text-foreground uppercase tracking-tight">Female: {femalePerc.toFixed(1)}%</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-(--chart-slate)" />
                        <span className="text-[10px] font-black text-foreground uppercase tracking-tight">Other: {otherPerc.toFixed(1)}%</span>
                    </div>
                </div>
            </div>
        </div>
