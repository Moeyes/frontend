    ...
    if (error) {
        return (
            <div className="p-12 text-center text-error bg-error/5 rounded-2xl border border-error/10">
                ...
                <p className="font-black text-lg uppercase tracking-tight">Failed to load participations</p>
        ...
        <div className="space-y-6">
            <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden transition-all hover:shadow-md">
                <SectionHeader
                    ...
                    subtitle="Track participation across events and sports"
                    icon={Trophy}
                    action={
                            ...
                            accessor: (p) => (
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-xs text-foreground font-black uppercase tracking-tight">{p.sport_name || `Sport #${p.sport_id}`}</p>
                                    </div>
                                    {p.category_name && (
                                        <p className="text-[10px] font-bold text-muted-foreground ml-5.5 uppercase opacity-60">{p.category_name}</p>
                                    )}
                                </div>
                            ),
                        },
                        ...
                        ...(isAdmin ? [{
                            ...
                            accessor: (p: any) => (
                                <Button 
                                    ...
                                    disabled={isRemoving}
                                    className="text-muted-foreground hover:text-error hover:bg-error/5 transition-all"
                                >
                ...
                {totalPages > 1 && (
                    <div className="p-4 border-t border-border flex items-center justify-between bg-muted/20">
                        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">
                            Showing <span className="text-foreground">{currentPage * PAGE_SIZE + 1}</span> to <span className="text-foreground">{Math.min((currentPage + 1) * PAGE_SIZE, totalCount)}</span> of <span className="text-foreground">{totalCount}</span>
                        </p>
                        <div className="flex gap-2">
                            <Button 
                                ...
                                className="h-8 font-black uppercase"
                            >
                                Previous
                            </Button>
                            <Button 
