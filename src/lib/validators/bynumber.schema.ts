...
const sportCountSchema = z.object({
    ...
    athlete_male_count: z.coerce.number().int().min(0),
    athlete_female_count: z.coerce.number().int().min(0),
    leader_male_count: z.coerce.number().int().min(0),
    leader_female_count: z.coerce.number().int().min(0),
...
export type ByNumberFormData = z.infer<typeof byNumberSchema>;
