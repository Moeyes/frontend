 ...
 * 
 * Displays a searchable, filtered list of participant registrations
 */
...
export function ParticipantList() {
    ...
    // Filter by own organization for user1 (Province)
    ...
    if (error) {
        return (
            <div className="p-8 text-center text-error bg-error/10 rounded-xl border border-error/20">
                ...
                <p className="font-bold">Failed to load participants</p>
                    ...
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="bg-secondary/30 text-left border-b border-border">
                                ...
                                <th className="p-4 text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Event & Sport</th>
                                <th className="p-4 text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Role</th>
                            ...
                            {isLoading ? (
                                Array(5).fill(0).map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={5} className="p-8 h-16 bg-secondary/10"></td>
                                    </tr>
                                ))
                                ...
                                registrations.map((p) => (
                                    <tr key={p.id} className="hover:bg-secondary/10 transition-colors group">
                                        ...
                                        <td className="p-4">
                                            ...
                                            <p className="text-xs text-muted-foreground truncate max-w-[200px]">{p.event_name || `Event #${p.event_id}`}</p>
                                        </td>
                                        <td className="p-4">
                                            <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                                                p.role === 'athlete' ? 'bg-primary/10 text-primary' : 'bg-amber-100 text-amber-700'
                                            }`}>
                                                {p.role}
                                            </span>
                                            {p.leader_role && (
                                                <p className="text-[10px] text-muted-foreground mt-1 ml-1">{p.leader_role}</p>
                                            )}
                                        </td>
                ...
                {totalPages > 1 && (
                    <div className="p-4 border-t border-border flex items-center justify-between bg-secondary/5">
                        <p className="text-xs text-muted-foreground font-medium">
                            Showing <span className="text-foreground">{currentPage * PAGE_SIZE + 1}</span> to <span className="text-foreground">{Math.min((currentPage + 1) * PAGE_SIZE, totalCount)}</span> of <span className="text-foreground">{totalCount}</span> participants
                        </p>
